...
export const CatSprite = ({ position, rotation, onMove }) => {
  const previousPositionRef = useRef(position); // Keep track of the last valid position
    ...
    collect: (monitor) => ({
      ...
      if (delta) {
        ...
        if (newX >= 0 && newY >= 0 && newX <= window.innerWidth && newY <= window.innerHeight) {
          onMove({ x: newX, y: newY }); // Update position if within bounds
          previousPositionRef.current = { x: newX, y: newY }; // Update last valid position
        } else {
          // Revert to last valid position if out of bounds
          onMove(previousPositionRef.current);
        }
      }
    },
  }));
  return (
    <div
      ...
      style={{
        ...
        right: `calc(50% +${position.y}px- 25px)`,
        width: '50px',
        height: '50px',
    ...
    <svg
      ...
      version="1.1"
      xmlSpace="preserve"
      style={{ transform: `translate(${position.x}px , ${position.y}px)`, transition:"transform 0.3s",}}
    >
      <g>
